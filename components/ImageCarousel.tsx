...
export default function ImageCarousel(media: { data: ImageType[] }) {
  if (media.data.length > 3) {
    return (
      <div
        className="group flex w-full max-w-4xl flex-nowrap gap-2 overflow-hidden
        ...
        <div className="group-hover:pause flex animate-loop-scroll items-center justify-center gap-2 md:justify-start [&_img]:max-w-none">
          {media.data.map((media) => (
            <Image
              ...
              className="h-32 w-32 rounded object-cover md:h-60 md:w-60"
            />
          ))}
        </div>
        <div className="group-hover:pause flex animate-loop-scroll items-center justify-center gap-2 md:justify-start [&_img]:max-w-none">
      ...
      <div className="flex flex-row flex-wrap justify-center gap-2">
        {media.data.map((media) => (
          <Image
            ...
            height={225}
            key={media.attributes.url}
            className="z-0 h-32 w-32 rounded object-cover transition-all delay-100 duration-200 ease-in-out
